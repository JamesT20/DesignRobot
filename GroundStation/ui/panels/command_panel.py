...
class CommandPanel(tk.Frame):
    def __init__(self, parent, tx_queue):
        ...
        tk.Label(self, text="Left Motor").grid(row=0, column=0, padx=5, pady=5)
        tk.Scale(self, from_=-100, to=100, variable=self._left_var, orient="vertical").grid(row=1, column=0, padx=10)

        tk.Label(self, text="Right Motor").grid(row=0, column=1, padx=5, pady=5)
        tk.Scale(self, from_=-100, to=100, variable=self._right_var, orient="vertical").grid(row=1, column=1, padx=10)

        tk.Button(self, text="Send Speed", command=self._send_speed).grid(row=2, column=0, columnspan=2, pady=5)
        ...
        tk.Button(self, text="Reboot", command=self._reboot).grid(row=4, column=0, columnspan=2, pady=2)
